...
function get_text_input($value){
	$opt = get_option( $value['id'] );
	$val =  $opt != "" ? stripslashes($opt) : stripslashes($value['std']);

	// if this is a slide title, html encode quotes
	if(preg_match('/slide[a-z]+?_title$/', $value['id'])){
...
$options = array (
	...
	array("name" => __("Enable autostart?", 'product'),
		"description" => __("You can enable autostart slideshow, the default is on", 'product'),
		"id" => $shortname . $shortprefix . "slide_auto",
		"inblock" => "slideone",
	    "type" => "select",
		"std" => "Show",
		"options" => array("yes", "no")),
	  ...
	  <ul class="ui-tabs-nav" id="tabm">
		...
		<?php $value_var = 'slideone'; foreach ($options as $value) { ?>
						...
						<div class="description"><?php echo $value['name']; ?><br /><span>	<?php
							...
							?></span></div>
						<div class="input-option"><p><?php echo get_text_input($value); ?></p></div>
						</div>
						...
						echo $value['description']; }
						?></span></div>
						<div class="input-option"><p><textarea name="<?php echo $valuey; ?>" class="mytext" cols="40%" rows="8" /><?php if ( get_option($valuey) != "") { echo trim(stripslashes($video_code)); } else { echo trim($value['std']); } ?></textarea></p></div>
						</div>
		...
		<?php $value_var = 'branding'; foreach ($options as $value) { ?>
						...
						<div class="description"><?php echo $value['name']; ?><br /><span>	<?php
								if (isset($value['description'])){
							echo $value['description']; }
							if ( get_option( $value['id'] ) != "") { $val= stripslashes(get_option( $value['id']) ); }
							else if(isset($value['std'])){ $val = stripslashes($value['std']); }
							else $val = '';
							?></span></div>
						<div class="input-option"><p><input name="<?php echo $value['id']; ?>" class="myfield" id="<?php echo $value['id']; ?>" type="<?php echo $value['type']; ?>" value="<?php echo $val; ?>" /></p></div>
						</div>
						...
						if ( get_option($valuey) != "") { $val = stripslashes($video_code); }
						else if(isset($value['std'])){ $val = $value['std']; }
						else $val = '';?>
						<div class="description"><?php echo $value['name']; ?><br /><span><?php
							if (isset($value['description'])){
						echo $value['description']; }
						?></span></div>
						<div class="input-option"><p><textarea name="<?php echo $valuey; ?>" class="mytext" cols="40%" rows="8" /><?php echo trim($val); ?></textarea></p></div>
						</div>
		...
		<input name="reset" type="submit" class="sbutton resetButton" value="<?php echo esc_attr(__('Reset All Options','product')); ?>" />
		...
		array(
			...
			"id" => $shortname . $shortprefix . "slideshow_tabs_border_link",
			"inblock" => "slideshow",
			"std" => "",
			"type" => "colorpicker"),
	...
	<?php 	wp_enqueue_script("jquery"); ?>
		...
		jQuery(document).ready(function($){
			...
			$('.resetButton').click(function(e){
				var con = confirm("Are you sure you want to reset? All saved theme options will be lost.");
				if(con)
					return true;
				else{
					return false;
