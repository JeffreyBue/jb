 ...
 */
function bp_dtheme_ajax_querystring_product( $query_string, $object ) {
	global $bp;
	...
	return apply_filters( 'bp_dtheme_ajax_querystring_product', $query_string, $object, $object_filter, $object_scope, $object_page, $object_search_terms, $object_extras );
}
add_filter( 'bp_ajax_querystring', 'bp_dtheme_ajax_querystring_product', 10, 2 );

/* This function will simply load the template loop for the current object. On an AJAX request */
function bp_dtheme_object_template_loader_product() {
...
add_action( 'wp_ajax_members_filter', 'bp_dtheme_object_template_loader_product' );
add_action( 'wp_ajax_groups_filter',  'bp_dtheme_object_template_loader_product' );
add_action( 'wp_ajax_blogs_filter',   'bp_dtheme_object_template_loader_product' );
add_action( 'wp_ajax_forums_filter',  'bp_dtheme_object_template_loader_product' );

// This function will load the activity loop template when activity is requested via AJAX
function bp_dtheme_activity_template_loader_product() {
	global $bp;
...
add_action( 'wp_ajax_activity_widget_filter', 'bp_dtheme_activity_template_loader_product' );
add_action( 'wp_ajax_activity_get_older_updates', 'bp_dtheme_activity_template_loader_product' );

/* AJAX update posting */
function bp_dtheme_post_update_product() {
	global $bp;
	...
	if ( empty( $_POST['content'] ) ) {
		echo '-1<div id="message" class="error"><p>' . __( 'Please enter some content to post.', 'product' ) . '</p></div>';
		return false;
	}
	...
	if ( empty( $activity_id ) ) {
		echo '-1<div id="message" class="error"><p>' . __( 'There was a problem posting your update, please try again.', 'product' ) . '</p></div>';
		return false;
	}
...
add_action( 'wp_ajax_post_update', 'bp_dtheme_post_update_product' );

/* AJAX activity comment posting */
function bp_dtheme_new_activity_comment_product() {
	global $bp;
	...
	if ( empty( $_POST['content'] ) ) {
		echo '-1<div id="message" class="error"><p>' . __( 'Please do not leave the comment area blank.', 'product' ) . '</p></div>';
		return false;
	}

	if ( empty( $_POST['form_id'] ) || empty( $_POST['comment_id'] ) || !is_numeric( $_POST['form_id'] ) || !is_numeric( $_POST['comment_id'] ) ) {
		echo '-1<div id="message" class="error"><p>' . __( 'There was an error posting that reply, please try again.', 'product' ) . '</p></div>';
		return false;
	}
...
add_action( 'wp_ajax_new_activity_comment', 'bp_dtheme_new_activity_comment_product' );

/* AJAX delete an activity */
function bp_dtheme_delete_activity_product() {
	global $bp;
	...
	if ( !bp_activity_delete( array( 'id' => $activity->id, 'user_id' => $activity->user_id ) ) ) {
		echo '-1<div id="message" class="error"><p>' . __( 'There was a problem when deleting. Please try again.', 'product' ) . '</p></div>';
		return false;
	}
...
add_action( 'wp_ajax_delete_activity', 'bp_dtheme_delete_activity_product' );

/* AJAX delete an activity comment */
function bp_dtheme_delete_activity_comment_product() {
	global $bp;
...
add_action( 'wp_ajax_delete_activity_comment', 'bp_dtheme_delete_activity_comment_product' );
...
add_action( 'wp_ajax_activity_mark_fav', 'bp_dtheme_mark_activity_favorite_product' );
...
add_action( 'wp_ajax_activity_mark_unfav', 'bp_dtheme_unmark_activity_favorite_product' );
 ...
 * @since 1.5
 */
function bp_dtheme_get_single_activity_content_product() {
	...
	do_action_ref_array( 'bp_dtheme_get_single_activity_content_product', array( &$activity ) );
...
add_action( 'wp_ajax_get_single_activity_content', 'bp_dtheme_get_single_activity_content_product' );

/* AJAX invite a friend to a group functionality */
function bp_dtheme_ajax_invite_user_product() {
	global $bp;
	...
	if ( 'invite' == $_POST['friend_action'] ) {
		...
		echo '<div class="action">
				<a class="button remove" href="' . wp_nonce_url( $bp->loggedin_user->domain . bp_get_groups_slug() . '/' . $_POST['group_id'] . '/invites/remove/' . $user->id, 'groups_invite_uninvite_user' ) . '" id="uid-' . esc_attr( $user->id ) . '">' . __( 'Remove Invite', 'product' ) . '</a>
			  </div>';
		echo '</li>';
...
add_action( 'wp_ajax_groups_invite_user', 'bp_dtheme_ajax_invite_user_product' );

/* AJAX add/remove a user as a friend when clicking the button */
function bp_dtheme_ajax_addremove_friend_product() {
	global $bp;
	...
	if ( 'is_friend' == BP_Friends_Friendship::check_is_friend( $bp->loggedin_user->id, $_POST['fid'] ) ) {
		...
		if ( !friends_remove_friend( $bp->loggedin_user->id, $_POST['fid'] ) ) {
			echo __("Friendship could not be canceled.", 'product');
		} else {
			echo '<a id="friend-' . $_POST['fid'] . '" class="add" rel="add" title="' . __( 'Add Friend', 'product' ) . '" href="' . wp_nonce_url( $bp->loggedin_user->domain . bp_get_friends_slug() . '/add-friend/' . $_POST['fid'], 'friends_add_friend' ) . '">' . __( 'Add Friend', 'product' ) . '</a>';
		}
		...
		if ( !friends_add_friend( $bp->loggedin_user->id, $_POST['fid'] ) ) {
			echo __("Friendship could not be requested.", 'product');
		} else {
			echo '<a href="' . $bp->loggedin_user->domain . bp_get_friends_slug() . '/requests" class="requested">' . __( 'Friendship Requested', 'product' ) . '</a>';
		}
	} else {
		echo __( 'Request Pending', 'product' );
	}

	return false;
}
add_action( 'wp_ajax_addremove_friend', 'bp_dtheme_ajax_addremove_friend_product' );

/* AJAX accept a user as a friend when clicking the "accept" button */
function bp_dtheme_ajax_accept_friendship_product() {
	check_admin_referer( 'friends_accept_friendship' );

	if ( !friends_accept_friendship( $_POST['id'] ) )
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem accepting that request. Please try again.', 'product' ) . '</p></div>';

	return true;
}
add_action( 'wp_ajax_accept_friendship', 'bp_dtheme_ajax_accept_friendship_product' );

/* AJAX reject a user as a friend when clicking the "reject" button */
function bp_dtheme_ajax_reject_friendship_product() {
	check_admin_referer( 'friends_reject_friendship' );

	if ( !friends_reject_friendship( $_POST['id'] ) )
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem rejecting that request. Please try again.', 'product' ) . '</p></div>';

	return true;
}
add_action( 'wp_ajax_reject_friendship', 'bp_dtheme_ajax_reject_friendship_product' );

/* AJAX join or leave a group when clicking the "join/leave" button */
function bp_dtheme_ajax_joinleave_group_product() {
	global $bp;
	...
	if ( !groups_is_user_member( $bp->loggedin_user->id, $group->id ) ) {
		...
		if ( 'public' == $group->status ) {
			...
			if ( !groups_join_group( $group->id ) ) {
				_e( 'Error joining group', 'product' );
			} else {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="leave-group" rel="leave" title="' . __( 'Leave Group', 'product' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'leave-group', 'groups_leave_group' ) . '">' . __( 'Leave Group', 'product' ) . '</a>';
			}
			...
			if ( !groups_send_membership_request( $bp->loggedin_user->id, $group->id ) ) {
				_e( 'Error requesting membership', 'product' );
			} else {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="membership-requested" rel="membership-requested" title="' . __( 'Membership Requested', 'product' ) . '" href="' . bp_get_group_permalink( $group ) . '">' . __( 'Membership Requested', 'product' ) . '</a>';
			}
		}
		...
		if ( !groups_leave_group( $group->id ) ) {
			_e( 'Error leaving group', 'product' );
		} else {
			if ( 'public' == $group->status ) {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="join-group" rel="join" title="' . __( 'Join Group', 'product' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'join', 'groups_join_group' ) . '">' . __( 'Join Group', 'product' ) . '</a>';
			} else if ( 'private' == $group->status ) {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="request-membership" rel="join" title="' . __( 'Request Membership', 'product' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'request-membership', 'groups_send_membership_request' ) . '">' . __( 'Request Membership', 'product' ) . '</a>';
			}
		}
	}
}
add_action( 'wp_ajax_joinleave_group', 'bp_dtheme_ajax_joinleave_group_product' );

/* AJAX close and keep closed site wide notices from an admin in the sidebar */
function bp_dtheme_ajax_close_notice_product() {
	global $userdata;

	if ( !isset( $_POST['notice_id'] ) ) {
		echo "-1<div id='message' class='error'><p>" . __('There was a problem closing the notice.', 'product') . '</p></div>';
	} else {
...
add_action( 'wp_ajax_messages_close_notice', 'bp_dtheme_ajax_close_notice_product' );

/* AJAX send a private message reply to a thread */
function bp_dtheme_ajax_messages_send_reply_product() {
	global $bp;
	...
	if ( $result ) { ?>
		<div class="message-box new-message">
			<div class="message-metadata">
				...
				<strong><a href="<?php echo $bp->loggedin_user->domain ?>"><?php echo $bp->loggedin_user->fullname ?></a> <span class="activity"><?php printf( __( 'Sent %s', 'product' ), bp_core_time_since( bp_core_current_time() ) ) ?></span></strong>
	...
	<?php
		...
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem sending that reply. Please try again.', 'product' ) . '</p></div>';
	}
}
add_action( 'wp_ajax_messages_send_reply', 'bp_dtheme_ajax_messages_send_reply_product' );

/* AJAX mark a private message as unread in your inbox */
function bp_dtheme_ajax_message_markunread_product() {
	global $bp;

	if ( !isset($_POST['thread_ids']) ) {
		echo "-1<div id='message' class='error'><p>" . __('There was a problem marking messages as unread.', 'product' ) . '</p></div>';
	} else {
...
add_action( 'wp_ajax_messages_markunread', 'bp_dtheme_ajax_message_markunread_product' );

/* AJAX mark a private message as read in your inbox */
function bp_dtheme_ajax_message_markread_product() {
	global $bp;

	if ( !isset($_POST['thread_ids']) ) {
		echo "-1<div id='message' class='error'><p>" . __('There was a problem marking messages as read.', 'product' ) . '</p></div>';
	} else {
		$thread_ids = explode( ',', $_POST['thread_ids'] );
...
add_action( 'wp_ajax_messages_markread', 'bp_dtheme_ajax_message_markread_product' );

/* AJAX delete a private message or array of messages in your inbox */
function bp_dtheme_ajax_messages_delete_product() {
	global $bp;

	if ( !isset($_POST['thread_ids']) ) {
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem deleting messages.', 'product' ) . '</p></div>';
	} else {
		$thread_ids = explode( ',', $_POST['thread_ids'] );
		...
		_e( 'Messages deleted.', 'product' );
	}
}
add_action( 'wp_ajax_messages_delete', 'bp_dtheme_ajax_messages_delete_product' );

/**
 * bp_dtheme_ajax_messages_autocomplete_results_product()
 *
 ...
 * @return none
 */
function bp_dtheme_ajax_messages_autocomplete_results_product() {
	global $bp;
...
add_action( 'wp_ajax_messages_autocomplete_results', 'bp_dtheme_ajax_messages_autocomplete_results_product' );
