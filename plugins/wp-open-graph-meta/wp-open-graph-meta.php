...
class de_omaxis_wp_open_graph_meta
{
    ...
    protected function _add_image()
    {
				...
				$FBimage = get_the_post_thumbnail($post->ID, array(200,200));
				preg_match_all('/<img.+src=[\'"]([^\'"]+)[\'"].*>/i', $FBimage, $imgArray);
				$imgSrc = $imgArray[1][0];
						$this->_metas['og:image'] = $imgSrc;
				}else {
				$imgSrc = get_bloginfo('template_url')."/screenshot.jpg";
				$this->_metas['og:image'] = $imgSrc;
				}
    }
